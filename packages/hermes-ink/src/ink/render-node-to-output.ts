...
function renderNodeToOutput(
  ...
  if (yogaNode) {
    ...
    // PATCH(wxnodus): 子树含未缓存（新插入）节点时父不 blit——React 19
    // 并发插入 overlay 内容（modelPicker/sessions 等）时父链 dirty 可能
    // 未传播，blit 会跳过整个子树导致新内容不显示。
    const hasUncachedDescendant = (n: DOMElement): boolean => {
      for (const c of n.childNodes) {
        const el = c as DOMElement
        if (!nodeCache.get(el)) return true
        if (hasUncachedDescendant(el)) return true
      }
      return false
    }

    if (
      process.env.WXNODUS_NOBLIT !== '1' &&
      !hasUncachedDescendant(node) &&
      !node.dirty &&
      !skipSelfBlit &&
      node.pendingScrollDelta === undefined &&
      cached &&
    ...
    if (height === 0 && siblingSharesY(node, yogaNode)) {
      // PATCH(wxnodus): absolute 节点（overlay）首帧可能因布局时序 h=0 被
      // 跳过——若缓存 0 尺寸 rect，后续帧布局恢复（h>0）时 blit 条件
      // (cached.w/h === 当前 w/h) 不匹配会正常重绘；但更糟的是父链
      // hasUncachedDescendant 检查看到「全子树都有缓存」而走 blit 恢复
      // prevScreen（首帧没画过 overlay）→ overlay 永不显示。这里对
      // absolute 节点不写缓存，强制后续帧走完整递归渲染。
      if (node.style.position === 'absolute') {
