...
const applyDisplayStyles = (node: LayoutNode, style: Styles): void => {
  // PATCH(wxnodus): display 从 'none' 切回默认时 props 里不再有 display 键，
  // 原逻辑 `'display' in style` 不触发 setDisplay，yoga 停留在 None 导致
  // 节点永不显示。这里无条件同步：键缺失 = 恢复默认 flex。
  node.setDisplay(style.display === 'none' ? LayoutDisplay.None : LayoutDisplay.Flex)
}
