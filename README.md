...
> **平台声明（用户决策，W6-09）**：本产品**只做 Windows 本地 CLI**——npm 包显式 `os: ["win32"]`，
> 验收与发行证据仅覆盖 Windows（Gate I `windows-only` 档，非 Windows cells 声明性豁免）；
> Linux/macOS 未验证、不宣称支持，Windows 专属能力（UIA/语音/桌面控制）在非 Windows 平台不承诺可用。
...
- **零配置离线可用**——不是「支持本地模型」：不配置任何 key/模型，规则脑（60 个需求模板）也能离线编译、确定性计算毫秒级响应
...
**规则脑**（零配置兜底）：60 个需求模板（待办/记账/笔记/简历/日程/商城/题库/考勤/宠物/租车/招聘/捐赠/票务/家教/维修/志愿者/讲座/外卖/预约/租借/家政……）不依赖任何模型，离线编译；未命中且无密钥时**诚实引导配置**，绝不假装回答。`/calc` 等确定性能力同样毫秒级本地执行。
...
- /balance threshold <数值> 低余额预警阈值（默认 5）+ /balance auto-stop [on|off] 余额耗尽硬停（实测 0 即停止后续对话，充值自动恢复）；会话预算硬停：/config set budgetTokens <n> budgetStop true；自然语言直达：「成本多少」「花了多少钱」→ /cost、「余额还有多少」→ /balance status
...
| 「看看这张图 …」 | GLM-4V 视觉理解 |
| 「体检」 | 系统健康检查 |
| 「成本多少 / 花了多少钱」 | 成本估算（/cost，会话/今日/7天/30天） |
...
**副线 3：本地能力**（无 key 全可用）——`/search` 自研 DDG/Bing 双引擎联网搜索（免 key，`--content [N]` 搜索即读：对前 N 条结果抓取正文）· `/claw` 网页抓取（SSRF 三层防护：内网/IPv6/NAT64 变体 + DNS 重绑定 + 重定向逐跳；readability 式正文提取 + JS 页 Playwright 无头渲染兜底）· `/browser` 浏览器自动化（有头 Edge/Chrome，AI 可导航/点击/输入/截图）· `/computer` 桌面控制：**UIA 元素级**（Windows UI Automation 零依赖桥：窗口枚举/控件树/按名称或 AutomationId 定位/原生 Invoke 点击/ValuePattern 中文输入）+ robotjs 坐标层（DPI 换算 + 动作护栏），`computer_observe` 截图理解闭环 · `/video` 视频抽帧分析 · `/vision`/`/img` 图片理解 · `/capture` 截屏留证。**视觉开放通道**：默认智谱 glm-4v-flash（免费），`settings.visionBaseURL/visionModel/visionKey` 或环境变量可换任意 OpenAI 兼容端点（ollama 本地 qwen2.5-vl / OpenRouter / 自建网关），`visionLocal=true` 启用 transformers.js 本地 VLM（moondream2，完全离线无 key），失败可归因（无 key/端点被拒/网络）。
...
- ✅ 全命令扫描 125/125 可用（scripts/cmd-sweep.mjs 回归工具；注册表 109 条命令全覆盖）
