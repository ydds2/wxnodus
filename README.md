...
> **概念进 · 证据出**：说一句话，交付可运行系统——全程证据留痕，无 key 也能用。

Windows 本地优先的 AI agent CLI——数据不出机、模型可不出机。把「需求」当源代码一样编译：**规格 → 分解 → 执行 → 验证 → 证据**五段闭环，每段都可审计、可回滚、可重放。这是 WxNodus 自己的方法论（编译学派），不是又一个聊天框。
...
**规则脑**（无 key 兜底）：30 个需求模板（待办/记账/笔记/动画/通用……）不依赖任何模型，离线编译；未命中且无密钥时**诚实引导配置**，绝不假装回答。`/calc` 等确定性能力同样毫秒级本地执行。
...
**副线 3：本地能力**（无 key 全可用）——`/search` 自研 DDG/Bing 双引擎联网搜索（免 key）· `/claw` 网页抓取（SSRF 三层防护：内网/IPv6/NAT64 变体 + DNS 重绑定 + 重定向逐跳）· `/browser` 浏览器自动化（有头 Edge/Chrome，AI 可导航/点击/输入/截图）· `/computer` 桌面控制（截图 → GLM-4V 理解 → 坐标点击/键入/打开，DPI 换算 + 动作护栏）· `/video` 视频抽帧分析 · `/vision`/`/img` 图片理解 · `/capture` 截屏留证。
