...
| `/doctor` `/help` | 体检 / 命令总览 |
| `/brand` | 「独一无二」命名（set 名称/图标——品牌行与欢迎语） |

## 独一无二与 SDK

- **实例身份**：每份 wxnodus 首次启动自动生成本机唯一身份（离线随机，绝不联网登记）——
  TUI 品牌行/欢迎语以专属代号示人（`/status` 可查；`/brand set` 自定义命名完全胜出）。
  同机多份安装（`--data-dir` 隔离）身份互异。
- **@wxnodus/sdk**（私有化/嵌入）：`launchWxnodus()` 拉起本地网关（127.0.0.1 + stdout 随机 token
  握手——零云端），`rpc()`（chat/command/memory/sessions/identity）+ `events()` SSE + 审批应答闭环。
...
| 「搜一下我之前说的黑洞」 | 黑洞引擎检索 |
| 「算一下 2+3*4」 | 确定性计算（毫秒级不走模型——需 `WXNODUS_LEGACY_OFFLINE=1`；默认走 AI 层） |
| 「体检」 | 系统健康检查 |
