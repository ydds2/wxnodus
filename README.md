...
- **安全注入通道**（`/security`）：sudo 密码 / `$WXNODUS_SECRET_<NAME>` 环境变量密钥注入——敏感内容仅用户亲手输入（UI overlay）、仅内存使用（stdin 传密码不进进程列表）、绝不落盘/不进历史/不进模型上下文；`/security sudo|secret on|off` 控制通道，**关闭即同步清除内存缓存**（`all off` 全量清空）；**10 分钟未用自动过期**（`/security secrets list` 查看字段名与剩余有效期，不显示值）；**工具输出回填模型前按 vault 值精确脱敏**；表单请求写审计留痕（不含值）；hooks 子进程环境净化不含敏感值（hooks 不可引用 $WXNODUS_SECRET_*）；默认关闭
