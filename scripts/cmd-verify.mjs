...
record(`/help 面板命令抽查 ${EXPECT.length - missing.length - 3}/${EXPECT.length}（3 个截断命令展开验证）`, missing.length === 0, missing.length ? `缺失: ${missing.join(' ')}` : '')
record('/help 含分类图标(◈)', help.includes('◈'))
record('/help 含(☆)', help.includes('☆'))
record('/help 含截断提示(…)', help.includes('…'))
record('/help 含滚动提示(11 lines)', help.includes('lines'))
// 截断命令展开验证
await submit('/help /bench')
const hb = strip(out)
record('展开/bench 含描述', hb.includes('/bench') && (hb.includes('基准') || hb.includes('bench')))
await submit('/help /delegate')
const hd = strip(out)
record('展开/delegate 含描述', hd.includes('/delegate'))
await submit('/help /goal')
const hg = strip(out)
record('展开/goal 含描述', hg.includes('/goal'))

// ── 2. 建议补全 ──
p.write('/'); await sleep(700)
const sug = strip(out)
// 建议列表最多返回前 12 个（completeSlash 截断）——只查前部命令
record('建议含/help', sug.includes('/help'))
record('建议含/model', sug.includes('/model'))
await typeKeys('mem'); await sleep(600)
const sug2 = strip(out)
record('过滤mem后含/memory', sug2.includes('/memory'))
record('过滤mem后不含/calc', !sug2.includes('/calc'))
p.write('\x1b'); await sleep(300)

// ── 3. 执行链路抽查 ──
await submit('/version')
record('执行/version 含版本', strip(out).includes('3.0.0') || strip(out).includes('v3'))
p.write('\x03'); await sleep(900)
await submit('/memory')
record('执行/memory 含记忆', strip(out).includes('记忆') || strip(out).includes('memory') || strip(out).includes('黑洞'))
p.write('\x03'); await sleep(900)
await submit('/clear')
await sleep(1200)
record('执行/clear 后命令面板残留', !strip(out).slice(-1500).includes('/memory'))
