...
// 覆盖：/workspace show/set/reset（持久化、重启原子生效）；/download 真实下载落盘 + sha256 证据；
...
function makeCtx(dataDir: string, cwd: string, opts: { allowLoopback?: boolean } = {}) {
  ...
  codeIndex.install();
  const liveRoot = cwd;
  const liveSource = 'cwd';
  let pendingWorkspace: string | null | undefined;
  const ctx = {
    ...
    setWorkspace: (dir: string | null) => { pendingWorkspace = dir; },
    codeIndex,
  ...
  return { db, bus, ctx, getPendingWorkspace: () => pendingWorkspace };
}

describe('W7 命令层接线', () => {
  it('/workspace show/set/reset 只持久化，当前进程保持同一权威根', async () => {
    const dataDir = tmp();
    const cwd = tmp();
    const { db, bus, getPendingWorkspace } = makeCtx(dataDir, cwd);
    try {
      ...
      expect(String(set.output)).toContain('已保存');
      expect(String(set.output)).toContain('重启');
      expect(getPendingWorkspace()).toBe(setDir);
      const after = await bus.execute('/workspace');
      expect(String(after.output)).toContain(cwd);
      const reset = await bus.execute('/workspace reset');
      expect(String(reset.output)).toContain('重置已保存');
