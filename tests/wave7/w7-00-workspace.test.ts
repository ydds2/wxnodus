...
describe('resolveWorkspaceRoot 优先级与 fail-closed', () => {
  ...
  it('存在的文件不是工作区目录 → WORKSPACE_NOT_DIRECTORY fail-closed', () => {
    const file = join(tmp(), 'workspace.txt');
    writeFileSync(file, 'not a directory', 'utf8');
    const r = resolveWorkspaceRoot({ cli: file, cwd: tmp() });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error.code).toBe('WORKSPACE_NOT_DIRECTORY');
  });

  it('进入组合根前返回 realpath 规范路径', () => {
    const root = tmp();
    mkdirSync(join(root, 'child'));
    const r = resolveWorkspaceRoot({ cli: join(root, 'child', '..'), cwd: tmp() });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.value.value).toBe(realpathSync.native(root));
  });
