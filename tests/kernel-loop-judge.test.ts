...
const loopTool = () => ({
  loop_read: { schema: { type: 'function', function: { name: 'loop_read', description: 'loop', parameters: { type: 'object', properties: {} } } }, danger: false, canonical: { namespace: 'agent' as const, effectKind: 'filesystem.read' as const }, run: async () => 'same output every time' },
});
