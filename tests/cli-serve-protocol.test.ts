...
beforeAll(() => {
  ...
  const kernel: ServeKernel = {
    ...
    agent: {
      run: async (p) => ({ ok: true, text: `回复：${p}`, turns: 1, interrupted: false }),
      getSessionId: () => 'src1', setSessionId: () => {},
    },
    commandBus: { execute: async (c) => ({ ok: true, output: `执行：${c}` }) },
...
describe('serve 协议加固（supremacy 2.5）', () => {
  ...
  it('GET /events：chat RPC 完成后广播 session.changed（事件驱动刷新，无轮询）', async () => {
    ...
    const res = await fetch(`http://127.0.0.1:${PORT}/events`, { headers: auth, signal: ctrl.signal });
    ...
    await fetch(`http://127.0.0.1:${PORT}/rpc`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', ...auth },
      body: JSON.stringify({ method: 'chat', params: { prompt: '你好' } }),
    });
