...
describe('W1-05 ToolCatalog', () => {
  ...
  it('atomically swaps an owner and accepts an empty replacement', () => {
    const catalog = createToolCatalog();
    expect(catalog.register('mcp:a', [descriptor('mcp:read', 'mcp:a')]).ok).toBe(true);

    const swapped = catalog.swapOwner('mcp:a', [descriptor('mcp:write', 'mcp:a')]);
    expect(swapped).toMatchObject({ ok: true, value: { replaced: ['mcp:read'] } });
    expect(catalog.resolve('mcp:read')).toMatchObject({ ok: false, error: { code: 'TOOL_NOT_FOUND' } });
    expect(catalog.resolve('mcp:write').ok).toBe(true);

    expect(catalog.swapOwner('mcp:a', [])).toMatchObject({ ok: true, value: { replaced: ['mcp:write'] } });
    expect(catalog.list('mcp:a')).toEqual([]);
  });

  it('preserves the previous owner table when a swap collides', () => {
    const catalog = createToolCatalog();
    expect(catalog.register('mcp:a', [descriptor('mcp:read', 'mcp:a')]).ok).toBe(true);
    expect(catalog.register('plugin:b', [descriptor('plugin:write', 'plugin:b')]).ok).toBe(true);

    const collision = catalog.swapOwner('mcp:a', [descriptor('plugin:write', 'mcp:a')]);
    expect(collision).toMatchObject({ ok: false, error: { code: 'TOOL_ALREADY_REGISTERED' } });
    expect(catalog.resolve('mcp:read').ok).toBe(true);
    expect(catalog.resolve('plugin:write').ok).toBe(true);
  });
