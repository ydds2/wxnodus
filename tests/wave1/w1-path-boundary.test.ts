...
describe('workspace path boundary', () => {
  ...
  it('rejects unsafe Windows resource names before filesystem access', async () => {
    if (process.platform !== 'win32') return;
    const missingRoot = join(tmpdir(), `wxnodus-boundary-missing-${Date.now()}`);
    const result = await validateWorkspaceTarget(missingRoot, join(missingRoot, 'CON.txt'));
    expect(result).toEqual({ ok: false, code: OUTSIDE });
  });
  ...
  it.each([
    ...
    String.raw`\??\C:\workspace\file.txt`,
    String.raw`C:workspace\file.txt`,
  ])('rejects Windows namespace or drive-relative target %s', async target => {
    if (process.platform !== 'win32') return;
    const root = await mkdtemp(join(tmpdir(), 'wxnodus-boundary-resource-'));
    try {
      expect(await validateWorkspaceTarget(root, target)).toEqual({ ok: false, code: OUTSIDE });
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });

  it('accepts an ordinary long absolute workspace target', async () => {
    if (process.platform !== 'win32') return;
    const root = await mkdtemp(join(tmpdir(), 'wxnodus-boundary-long-'));
    try {
      const target = join(root, ...Array.from({ length: 28 }, (_, index) => `segment-${index}`), 'file.txt');
      expect(target.length).toBeGreaterThan(260);
      expect(await validateWorkspaceTarget(root, target)).toEqual({ ok: true, target: resolve(target) });
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
