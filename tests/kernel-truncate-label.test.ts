...
describe('delegate 子代理输出诚实截断', () => {
  ...
  it('继承当前 signal，且子代理失败不会伪装成成功文本', async () => {
    const { coreTools } = await import('../src/kernel/tools.js');
    const delegate = coreTools().delegate!;
    ...
    const ctx = {
      ...
      spawnSubagent: async (_goal: string, _depth: number | undefined, _def: unknown, execution: { signal?: AbortSignal }) => {
        seen = execution.signal;
        return { ok: false, turns: 1, output: '真实失败' };
      },
    };
    await expect(delegate.run({ goal: 'x' }, ctx as any)).rejects.toThrow('子代理未完成：真实失败');
...
describe('wx_cmd 命令输出诚实截断（labelTruncate 统一口径）', () => {
  ...
  it('把当前工具 signal 传给命令桥', async () => {
    const { coreTools } = await import('../src/kernel/tools.js');
    const controller = new AbortController();
    let seen: AbortSignal | undefined;
    await coreTools().wx_cmd!.run({ command: '/status' }, {
      signal: controller.signal,
      runCommand: async (_command: string, signal?: AbortSignal) => {
        seen = signal;
        return 'ok';
      },
    } as any);
