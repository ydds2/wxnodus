...
describe('cwd.set RPC（工作区作用域保护）', () => {
  it('拒绝运行中半切换，并提示经 /workspace 持久化后重启', async () => {
    ...
    try {
      ...
      expect(r.error).toContain('/workspace set')
      expect(kernel.cwd).toBe(origCwd)
      expect(setCwdCalls).toEqual([])
      expect(events.some(e => e.type === 'session.info')).toBe(false)
    } finally {
