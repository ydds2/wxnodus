...
describe('A21 safeFetchText 多方法（POST/PUT/DELETE）', () => {
  ...
  it('调用方取消会传到 fetch 并稳定返回取消错误', async () => {
    const origFetch = globalThis.fetch;
    ...
    try {
      ...
      const pending = safeFetchText('https://api.example.com/slow', { signal: controller.signal });
      await new Promise(resolve => setTimeout(resolve, 0));
      controller.abort('test-cancel');
      await expect(pending).resolves.toEqual({ error: '请求已取消' });
