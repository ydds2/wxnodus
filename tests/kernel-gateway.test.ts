...
function makeGateway(settings: Record<string, any> = {}) {
  ...
  const client = new GatewayClient(kernel as any);
  client.start();
  return client;
}
...
function makeForegroundHarness() {
  const bus = createEventBus(dir);
  const commandBus = createCommandBus();
  ...
  const agent = {
    ...
    run: async (prompt: string) => {
      runs++;
      bus.emit('agent.start', { prompt });
      bus.emit('agent.token', { text: 'owned-token' });
      markStarted();
      await new Promise<void>(resolve => { releaseRun = resolve; });
      bus.emit('agent.end', {});
      return { ok: true, text: 'done', turns: 1, interrupted: false };
    },
    abort: () => { aborts++; },
    steer: () => true,
    setMode() {},
  };
  commandBus.register('/probe', (_args, _raw, context) => {
...
describe('TUI 前台 Run 生命周期', () => {
  it('启动保留已恢复会话，不写回 default', () => {
    const h = makeForegroundHarness();
    expect((h.client as any).currentSessionId).toBe('restored-session');
    expect(h.agent.getSessionId()).toBe('restored-session');
  });

  it('异步预处理前原子占位，第二次提交 fail-closed', async () => {
    const h = makeForegroundHarness();
    const first = h.client.request('prompt.submit', { text: 'first' }) as Promise<any>;
    const second = await h.client.request('prompt.submit', { text: 'second' }) as any;
    expect(second).toMatchObject({ ok: false, code: 4009 });
    await expect(first).resolves.toMatchObject({ ok: true });
    await h.started;
    expect(h.getRuns()).toBe(1);
    h.release();
  });

  it('只投影当前 Run 事件，取消后等待匹配 run.final 才释放 busy', async () => {
    const h = makeForegroundHarness();
    const submit = await h.client.request('prompt.submit', { text: 'slow' }) as any;
    await h.started;
    h.bus.emit('agent.token', { text: 'foreign-token' });

    const interrupted = await h.client.request('session.interrupt', {}) as any;
    expect(interrupted).toMatchObject({ ok: true, interrupted: true });
    expect(h.getAborts()).toBe(1);
    expect((h.client as any).running).toBe(true);
    expect(h.events.filter(event => event.type === 'message.delta').map(event => event.payload.text)).toEqual(['owned-token']);

    h.release();
    for (let i = 0; i < 50 && (h.client as any).running; i++) {
      await new Promise(resolve => setTimeout(resolve, 2));
    }
    expect((h.client as any).running).toBe(false);
    const finals = h.bus.history().filter(event => event.type === 'run.final' && event.runId === submit.run_id);
    expect(finals).toHaveLength(1);
    expect(finals[0]?.payload?.status).toBe('cancelled');
  });

  it('command.dispatch 通过统一接纳端口产生权威终态', async () => {
    const h = makeForegroundHarness();
    const result = await h.client.request('command.dispatch', { name: 'probe' }) as any;
    expect(result).toEqual({ type: 'exec', output: 'probe-ok' });
    const finals = h.bus.history().filter(event => event.type === 'run.final');
    expect(finals).toHaveLength(1);
    expect(finals[0]).toMatchObject({ sessionId: 'restored-session', payload: { status: 'succeeded' } });
    expect((h.client as any).running).toBe(false);
  });

  it('session.activate 经同一接纳端口切换 Agent、Gateway 并产生唯一终态', async () => {
    const h = makeForegroundHarness();
    h.client['kernel'].adapter.data.sessions.create('target-session');

    const result = await h.client.request('session.activate', { session_id: 'target-session' }) as any;
    expect(result.session_id).toBe('target-session');
    expect(h.agent.getSessionId()).toBe('target-session');
    expect((h.client as any).currentSessionId).toBe('target-session');
    const finals = h.bus.history().filter(event => event.type === 'run.final');
    expect(finals).toHaveLength(1);
  ...
  it('活跃前台 Run 期间会话删除在数据库副作用前以 4009 拒绝', async () => {
    const h = makeForegroundHarness();
    h.client['kernel'].adapter.data.sessions.create('protected-session');
    await h.client.request('prompt.submit', { text: 'slow' });
    await h.started;

    const rejected = await h.client.request('session.delete', { session_id: 'protected-session' }) as any;
    expect(rejected).toMatchObject({ ok: false, code: 4009 });
    expect(h.client['kernel'].adapter.data.sessions.exists('protected-session')).toBe(true);
    h.release();
    expect(await waitFor(() => !(h.client as any).running)).toBe(true);
  });

  it('审批阶段取消使接纳失效，迟到允许不得启动 Run', async () => {
    const h = makeForegroundHarness();
    const pending = h.client.request('slash.exec', { command: 'deploy' }) as Promise<any>;
    for (let i = 0; i < 50 && !h.events.some(event => event.type === 'approval.request'); i++) {
      await new Promise(resolve => setTimeout(resolve, 2));
    }
    expect(h.events.some(event => event.type === 'approval.request')).toBe(true);

    await expect(h.client.request('session.interrupt', {})).resolves.toMatchObject({ ok: true, interrupted: true });
    await expect(h.client.request('approval.respond', { choice: 'allow' })).resolves.toEqual({ ok: true });
    await expect(pending).resolves.toMatchObject({ output: '', warning: expect.stringContaining('危险命令已取消') });

    expect(h.getRuns()).toBe(0);
...
describe('TUI 后台任务隔离', () => {
  it('prompt.background 经 TaskRunner 执行并投影真实 success，不调用共享 Agent', async () => {
    const bus = createEventBus(dir);
    let sharedRuns = 0;
    let childGoal = '';
    const agent = {
      run: async () => { sharedRuns++; return { ok: true, text: '', turns: 0, interrupted: false }; },
      abort() {}, setMode() {}, getMode: () => 'smart',
    ...
    const runner = createTaskRunner({
      ...
      spawnSubagent: async (goal) => {
        childGoal = goal;
        return { ok: true, output: '后台完成', turns: 1 };
      },
    });
    const g = new GatewayClient({
      dataDir: dir, cwd: process.cwd(), config: { get: () => ({}) }, bus,
      settings: { model: 'x' }, commandBus: createCommandBus(), taskRunner: runner,
      runInvocation: createRunInvocationPort({
        coordinator: createSessionRunCoordinator({ agent, bus }), agent,
        executeCommand: async () => ({ ok: true, type: 'exec', output: '' }),
      }),
      adapter: createTuiPresentationAdapter({ db, agent: agent as never }),
      applyModel() {}, setMode() {}, setTheme() {}, setThinking() {}, requestExit() {},
    } as any);
    const events: any[] = [];
    g.on('event', event => events.push(event));
    g.start();
    g.drain();

    const result = await gre(g, 'prompt.background', { text: '后台检查' });
    expect(await waitFor(() => runner.get(result.task_id)?.status === 'success')).toBe(true);
    expect(sharedRuns).toBe(0);
    expect(childGoal).toBe('后台检查');
    expect(events.filter(event => event.type === 'background.complete')).toEqual([
      expect.objectContaining({
        type: 'background.complete',
        payload: expect.objectContaining({ task_id: result.task_id, text: '后台完成', status: 'success' }),
      }),
    ]);
  });

  it('process.stop 只取消本 Gateway 创建的后台任务，不中止前台或无关任务', async () => {
    const bus = createEventBus(dir);
    let foregroundAborts = 0;
    const releases = new Map<string, () => void>();
    const agent = {
      run: async () => ({ ok: true, text: '', turns: 0, interrupted: false }),
      abort: () => { foregroundAborts++; }, setMode() {}, getMode: () => 'smart',
      setSessionId() {}, getSessionId: () => 's1', steer: () => true,
    };
    const runner = createTaskRunner({
      db, bus, dataDir: dir, maxConcurrent: 2,
      spawnSubagent: (goal) => new Promise(resolve => {
        releases.set(goal, () => resolve({ ok: true, output: goal, turns: 1 }));
      }),
    });
    const g = new GatewayClient({
      dataDir: dir, cwd: process.cwd(), config: { get: () => ({}) }, bus,
    ...
    const unrelated = runner.run({ goal: '无关任务', kind: 'agent' });
    const owned = await gre(g, 'prompt.background', { text: 'TUI 后台' });
    expect(await waitFor(() => runner.get(unrelated)?.status === 'running' && runner.get(owned.task_id)?.status === 'running')).toBe(true);
    expect(await gre(g, 'process.stop', {})).toEqual({ killed: 1 });

    expect(foregroundAborts).toBe(0);
    expect(runner.get(owned.task_id)?.status).toBe('cancelled');
    expect(runner.get(unrelated)?.status).toBe('running');
    const completion = events.find(event => event.type === 'background.complete' && event.payload.task_id === owned.task_id);
    expect(completion?.payload).toMatchObject({ status: 'cancelled', error: '用户取消' });
    releases.get('TUI 后台')?.();
    releases.get('无关任务')?.();
  });
});
...
describe('delegation.pause 真实持久化', () => {
  it('manager 活动表驱动 status，interrupt 定向调用同一 manager', async () => {
    const bus = createEventBus(dir)
    ...
    const delegateManager = {
      ...
      stop,
      listActive: () => [{
        taskId: 'sub-1', processId: 4321, goal: 'inspect', worktreePath: 'C:/work/sub-1',
        startedAt: 123, status: 'running',
    ...
    const g = new GatewayClient({
      dataDir: dir, cwd: process.cwd(), config: { get: () => ({}) }, bus,
      settings: { model: 'x' }, commandBus: createCommandBus(), delegateManager,
    ...
    const status = await gre(g, 'delegation.status', {})
    expect(status.active).toContainEqual(expect.objectContaining({
      subagent_id: 'sub-1', process_id: 4321, worktree: 'C:/work/sub-1', status: 'running',
    }))
    const interrupted = await gre(g, 'subagent.interrupt', { subagent_id: 'sub-1' })
  ...
  it('delegate lifecycle events cross the foreground filter without clearing foreground busy state', async () => {
    const bus = createEventBus(dir)
    const g = new GatewayClient({
      dataDir: dir, cwd: process.cwd(), config: { get: () => ({}) }, bus,
    ...
    ;(g as any).foregroundRun = {
      context: createRunContext({ runId: 'command:1', correlationId: 'c', sessionId: 's1' }),
      cancel() {},
    }
    const child = createRunContext({ runId: 'delegate:sub-1', correlationId: 'c', sessionId: 's1' })

    bus.withinRun(child, () => {
      bus.emit('agent.subagent', { phase: 'start', subagent_id: 'sub-1', goal: 'inspect' })
      bus.emit('agent.subagent', { phase: 'complete', subagent_id: 'sub-1', goal: 'inspect', ok: true, output: 'done' })
      bus.emit('run.final', { status: 'succeeded' })
    })

    expect(events.filter(event => event.type.startsWith('subagent.')).map(event => event.type)).toEqual([
      'subagent.start',
      'subagent.complete',
    ])
    expect((g as any).running).toBe(true)
    g.kill('test')
  })

  it('pause updates the delegate gate without cancelling unrelated foreground work', async () => {
    const bus = createEventBus(dir)
    const abort = vi.fn()
    ...
    const g = new GatewayClient({
      ...
      settings: { model: 'x' }, commandBus: createCommandBus(),
      delegateManager: {
        start: vi.fn(), stop: vi.fn(), listActive: () => [], setPaused, isPaused: () => true, shutdown: vi.fn(),
      },
      adapter: createTuiPresentationAdapter({ db, agent: ({
        run: async () => ({ ok: true, text: '', turns: 0, interrupted: false }), abort, setMode() {}, getMode: () => 'smart',
        setSessionId() {}, getSessionId: () => 's1', steer: () => true, setDelegationPaused() {},
      }) as never }),
      applyModel() {}, setMode() {}, setTheme() {}, setThinking() {}, requestExit() {},
    } as any)
    const cancel = vi.fn()
    ;(g as any).running = true
    ;(g as any).foregroundRun = { cancel }

    expect(await gre(g, 'delegation.pause', { pause: true })).toEqual({ paused: true })
...
describe('子代理富事件分流（A25）', () => {
  it('活跃子代理 reasoning.delta → subagent.thinking 事件', async () => {
    const bus = createEventBus(dir)
    const g = new GatewayClient({
      dataDir: dir, cwd: process.cwd(), db, mem, config: { get: () => ({}) }, bus,
      settings: { model: 'x' }, commandBus: createCommandBus(),
    ...
    g.start()
    g.drain()
    const context = createRunContext({ sessionId: 's1' })
    ;(g as any).foregroundRun = { context }
    bus.withinRun(context, () => {
      bus.emit('agent.subagent', { goal: '探索', phase: 'start', session_id: 'sub-live', subagent_id: 'sub-live' })
      bus.emit('reasoning.delta', { text: '子代理思考中', session_id: 'sub-live' })
    })
    expect(events.some(e => e.type === 'subagent.thinking')).toBe(true)
    const ev = events.find(e => e.type === 'subagent.thinking')
    expect(ev.payload.subagent_id).toBe('sub-live')
  ...
  it('活跃子代理 agent.tool → subagent.tool 事件', async () => {
    const bus = createEventBus(dir)
    const g = new GatewayClient({
      dataDir: dir, cwd: process.cwd(), db, mem, config: { get: () => ({}) }, bus,
      settings: { model: 'x' }, commandBus: createCommandBus(),
    ...
    bus.withinRun(context, () => {
      bus.emit('agent.subagent', { goal: '探索', phase: 'start', session_id: 'sub-live', subagent_id: 'sub-live' })
      bus.emit('agent.tool', { name: 'fs_read', args: { path: 'x' }, phase: 'start', toolId: 't1', session_id: 'sub-live' })
    })
