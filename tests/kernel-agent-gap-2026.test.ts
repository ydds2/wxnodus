...
describe('并行工具调度（danger 读写门）', () => {
  it('纯只读批次并行执行（并发计数实证），结果按槽位保序', async () => {
    ...
    let call = 0;
    const agent = createPipelineAgent(base({
      extraTools: {
        read_a: { schema: { type: 'function', function: { name: 'read_a', description: 'a', parameters: { type: 'object', properties: {} } } }, danger: false, canonical: { namespace: 'agent', effectKind: 'filesystem.read' }, run: slowRead },
        read_b: { schema: { type: 'function', function: { name: 'read_b', description: 'b', parameters: { type: 'object', properties: {} } } }, danger: false, canonical: { namespace: 'agent', effectKind: 'filesystem.read' }, run: slowRead },
      },
