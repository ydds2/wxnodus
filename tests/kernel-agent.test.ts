...
describe('工具延迟加载', () => {
  ...
  it('关闭时全表注入（回归）', async () => {
    ...
    expect(names.length).toBe(32); // 全表（31 内置含 notify/browser_* 六件套/web_search/memory_update/memory_delete/repo_map/cron_create/credential_form/memory_search/find_files/wx_cmd/command_search - tool_search 未注册）
  });
});
