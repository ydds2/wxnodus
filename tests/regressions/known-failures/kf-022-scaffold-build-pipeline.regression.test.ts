...
// 落位 + plan.json 落盘），绝不绕过计划只消费 Spec；未显式传 plan 时规则脑分解兜底。
...
describe('KF-022 resolved: scaffold 由 BuildPlan 驱动', () => {
  it('显式传入 plan：模块按拓扑序落位，plan.json 落盘（真实消费证据）', () => {
    const dir = tmp();
    const plan = makePlan(spec.summary, { key: null });
  ...
  it('未传 plan：规则脑分解兜底（绝不绕过计划）', () => {
    const dir = tmp();
    const r = instantiate(spec, dir);
    expect(r.ok).toBe(true);
    expect(existsSync(join(dir, 'plan.json'))).toBe(true);
    const planJson = JSON.parse(readFileSync(join(dir, 'plan.json'), 'utf8')) as { order: string[] };
    expect(planJson.order.length).toBeGreaterThan(0);
    // 文件仍完整（模块落位 + 验证/文档/测试统一生成）
