...
import {
  classifyPipelineArgs,
  classifyWindowsPath,
  classifyWindowsResourceName,
  commandTouchesSystemPath,
} from '../../src/infrastructure/fs/windowsPathClassifier.js';
...
describe('W8-02 系统路径分类器', () => {
  it.each([
    [String.raw`\\server\share\file.txt`, 'unc'],
    [String.raw`//server/share/file.txt`, 'unc'],
    [String.raw`\\?\UNC\server\share\file.txt`, 'extended-unc'],
    [String.raw`//?/uNc/server/share/file.txt`, 'extended-unc'],
    [String.raw`\\?\C:\workspace\file.txt`, 'device-namespace'],
    [String.raw`\\.\PhysicalDrive0`, 'device-namespace'],
    [String.raw`\??\C:\workspace\file.txt`, 'nt-namespace'],
    [String.raw`\\??\C:\workspace\file.txt`, 'nt-namespace'],
    [String.raw`\Device\HarddiskVolume1\file.txt`, 'nt-namespace'],
    [String.raw`\GLOBAL??\C:\workspace\file.txt`, 'nt-namespace'],
    [String.raw`\\GLOBALROOT\Device\HarddiskVolume1`, 'nt-namespace'],
    [String.raw`\DosDevices\C:\workspace\file.txt`, 'nt-namespace'],
    [String.raw`C:\workspace\file.txt:secret`, 'alternate-data-stream'],
    [String.raw`C:/workspace/file.txt:$DATA`, 'alternate-data-stream'],
    [String.raw`C:workspace\file.txt`, 'drive-relative'],
    [String.raw`c:`, 'drive-relative'],
    [String.raw`C:\workspace\CON`, 'reserved-device-name'],
    [String.raw`C:\workspace\nul.txt`, 'reserved-device-name'],
    [String.raw`C:\workspace\AUX. `, 'reserved-device-name'],
    [String.raw`C:\workspace\COM1.log`, 'reserved-device-name'],
    [String.raw`C:\workspace\lpt9...   `, 'reserved-device-name'],
    [String.raw`C:\workspace\PRN.txt\child`, 'reserved-device-name'],
  ])('资源名策略拒绝 %s 为 %s', (target, expectedClass) => {
    expect(classifyWindowsResourceName(target)).toMatchObject({
      allowed: false,
      class: expectedClass,
    });
  });

  it('资源名策略允许普通绝对路径、非设备名前缀和长路径', () => {
    const longPath = 'C:\\workspace\\' + 'long-segment\\'.repeat(30) + 'file.txt';
    expect(longPath.length).toBeGreaterThan(260);
    for (const target of [
      String.raw`C:\workspace\ordinary.txt`,
      String.raw`C:\workspace\com10.txt`,
      String.raw`C:\workspace\console.txt`,
      longPath,
    ]) {
      expect(classifyWindowsResourceName(target)).toMatchObject({ allowed: true, class: 'ordinary' });
    }
  });
