...
- **conhost 滚动拉顶修复（2026-08-19）**：滚动快路径在无 DECSTBM 的 classic conhost 上会只标记底部 damage，顶部滚入内容因此未被绘制；现按 `scrollHint` 将整个滚动区域加入脏行集合并逐行重绘，避免流式输出中突然跳回首页/内容锁死。同步恢复右缘留空的末列保护（`screenWidth - 2`），规避 pending-wrap 行漂移。新增滚动区域契约测试。
