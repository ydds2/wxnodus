...
- **输出降噪（2026-08-19）**：① 状态栏 📊 点击轮换 token 区间不再向 transcript 注入「token 区间已切换」sys 消息（反馈仅在状态栏段内体现，对话流不再被状态噪声刷屏）；② 工具调用默认从逐条展开改为**折叠单行摘要「▾ Tool calls (N)」**（点击展开逐条，对标同类 CLI 紧凑输出；`/details tools expanded` 或 config 可恢复展开）。
