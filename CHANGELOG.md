...
## 代码卫生（2026-08-20，用户不可见工程项——详见 audit §14.00）

- **仓库瘦身**：移除入库的运行时 drill 数据库、外部插件状态、184 个 UIA fixture 编译产物与沙盒探针残留；根目录散装分析文档归位 docs/；磁盘垃圾（日志/临时目录/构建产物）安全子集清零——`data/` 用户数据与竞品研究内容不删。
- **规范**：移除死模块 lib/editorLaunch（P1-1 裁决后零引用）；22 个无头模块补职责注释；TODO 收口为明确 backlog 注释。
- **优化**：浮层重绘定时器合并（高频变更定时器数减半）；状态栏热力格预计算缓存（消除每帧数组分配）；21 项依赖核查全部在役。全量 2894 用例零回归、九步门禁 + 远程 CI 全绿。
