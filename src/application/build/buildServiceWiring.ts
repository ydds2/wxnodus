...
export function createProductionBuildWiring(input: ProductionBuildWiringInput): OperationResult<ProductionBuildWiring> {
  ...
  const completionInput = async (context: BuildCompletionContext): Promise<OperationResult<CompletionGateInput>> => {
    ...
    const artifact = { id: context.snapshot.verificationId ? `artifact-${context.snapshot.verificationId}` : 'artifact-build', sha256: sha('a') };
    ...
    for (const criterion of context.criteria) {
      const request: VerificationRequest = {
        ...
        execution: {
          command: { executable: 'verify', argv: [criterion.id], cwd: '', normalized: `verify ${criterion.id}`, timeoutMs: 30_000 },
          exit: { code: 0, signal: null, timedOut: false, aborted: false },
          stdout: { attachmentId: 'out', bytes: Buffer.from('') },
          stderr: { attachmentId: 'err', bytes: Buffer.from('') },
        },
      };
    ...
    const reviewRun: ReviewRun = {
      id: randomUUID(),
      runId: input.runId,
      maker: { actorId: input.makerActorId, contextHash: sha('7') },
      reviewer: { actorId: input.reviewerActorId, contextHash: sha('9') },
      artifact,
