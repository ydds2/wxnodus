...
import { classifyWindowsResourceName } from './windowsPathClassifier.js';
...
export async function validateWorkspaceTarget(root: string, target: string): Promise<PathBoundaryResult> {
  ...
  if (process.platform === 'win32') {
    if (!classifyWindowsResourceName(root).allowed || !classifyWindowsResourceName(target).allowed) {
      return { ok: false, code: 'BUILD_PATH_OUTSIDE_WORKSPACE' };
    }
  }
