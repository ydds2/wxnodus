...
export function buildSystemPrompt(opts: SysPromptOpts): string {
  ...
  const lines: string[] = [
    ...
    '7. 编译学派方法论（WxNodus 独有）：任务按「规格 → 分解 → 执行 → 验证 → 证据」五段推进——先明确规格（要做什么/验收条件），分解为可执行步骤，执行，然后验证（运行测试/启动探活/读回检查），最后给出证据（验证输出/测试结果/文件路径）。无法验证的交付必须明确标注「未验证」。这是"概念进·证据出"的完整闭环，任何声称完成的结论都必须有验证证据支撑。',
    '',
    `## 当前模式：${opts.mode}`,
