...
export const COMMAND_LEVELS: Record<string, CommandLevel> = {
  ...
  '/usage range': 'safe', '/balance status': 'safe', '/warp': 'safe', '/config export': 'safe', '/cost': 'safe',
  ...
  '/backup': 'confirm', '/export': 'confirm', '/workspace': 'safe', '/workspace set': 'confirm', '/workspace reset': 'confirm',
  ...
  '/balance': 'confirm', '/balance set': 'confirm', '/balance refresh': 'confirm', '/balance on': 'confirm', '/balance off': 'confirm', '/balance threshold': 'confirm',
...
// 别名表（中文自然语言）——审查修复：从 registry.ts 移入本文件（registry 依赖本文件，
// 原位置使本文件无法引用 resolveAlias，中文别名（/权限 yolo 等）在 wx_cmd 分级中
