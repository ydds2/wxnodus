...
async function fetchRobots(origin: string, signal?: AbortSignal): Promise<string> {
  const hit = robotsCache.get(origin);
  if (hit && Date.now() - hit.ts < ROBOTS_TTL) return hit.text;
  const r = await safeFetchText(`${origin}/robots.txt`, { maxBytes: 64_000, signal });
  const text = 'error' in r ? '' : r.text;
  if (!signal?.aborted) robotsCache.set(origin, { text, ts: Date.now() });
  return text;
}

/** 抓取前护栏：返回 block=禁止理由（应拦截）或 captcha=验证码页面提示 */
export async function robotsGuard(url: string, html: string, signal?: AbortSignal): Promise<{ block?: string; captcha?: boolean }> {
  const out: { block?: string; captcha?: boolean } = {};
  try {
    const u = new URL(String(url ?? ''));
    const robotsTxt = await fetchRobots(u.origin, signal);
