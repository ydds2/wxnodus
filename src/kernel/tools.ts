...
export interface ToolCtx {
  ...
  runCommand?: (input: string, signal?: AbortSignal) => Promise<string>;
}
...
export interface ToolDef {
  ...
  canonical?: {
    ...
    source?: string;
    effectKind?: import('../domain/effects/effectDescriptor.js').EffectKind;
  };
...
export function coreTools(): Record<string, ToolDef> {
  const fsRead: ToolDef = {
    ...
    async run({ path, offset, limit }, ctx) {
      try {
        ...
        const full = (await safeWorkspaceRead(ctx.cwd, p)).toString('utf8');
  ...
  const fsWrite: ToolDef = {
    ...
    async run({ path, content }, ctx) {
      try {
        ...
        try { before = await safeWorkspaceRead(ctx.cwd, p); } catch { /* new file */ }
        if (before) {
          try {
  ...
  const delegate: ToolDef = {
    ...
    async run({ goal, kind }, ctx) {
      ...
      const r = await ctx.spawnSubagent(
        String(goal ?? '').trim() || '（空任务）',
        undefined,
        subDef,
        { signal: ctx.signal },
      );
      if (!r.ok) {
        if (r.interrupted || r.status === 'cancelled' || ctx.signal?.aborted) {
          throw new DOMException('子代理已取消', 'AbortError');
        }
        throw new Error(`子代理未完成：${labelTruncate(String(r.output ?? ''), 1000, '子代理失败输出过长')}`);
      }
      return `子代理完成${def ? `（${def.label}型）` : ''}（${r.turns} 轮）：\n${labelTruncate(String(r.output ?? ''), 4000, '子代理输出过长——goal 里要求结论精简，或拆分子任务')}`;
    },
  };
  ...
  const wxCmd: ToolDef = {
    ...
    async run(args, ctx) {
      ...
      const out = await ctx.runCommand(command, ctx.signal);
