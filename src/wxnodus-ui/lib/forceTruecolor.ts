...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.WXNODUS_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep WxNodus on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via WXNODUS_TUI_TRUECOLOR=1.
