...
const logDir = join(process.env.WXNODUS_HOME?.trim() || join(homedir(), '.wxnodus'), 'logs')
...
// a real ~/.wxnodus (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('wxnodus-tui: parent lifecycle log unavailable\n')
    }
  }
}
