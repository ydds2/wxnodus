...
export const writeActiveSessionFile = (sessionId: null | string, file = process.env.WXNODUS_TUI_ACTIVE_SESSION_FILE) => {
  if (!file || !sessionId) {
    return
  }
...
export function useSessionLifecycle(opts: UseSessionLifecycleOptions) {
  ...
  const startNewSession = useCallback(
    async (msg?: string, title?: string, keepCurrent = false) => {
      ...
      // A25：未配置时提示继续（不阻塞，哲学不变）；
      // 此前 gateway 硬编码 true 掩盖真实状态，这里按真实值走提示路径
      if (setup?.provider_configured === false) {
        sys('未配置模型密钥——请用 /model set-key <密钥> 配置（配置类命令不受影响）')
      }
  ...
  const resumeById = useCallback(
    (id: string) => {
      ...
      rpc<SetupStatusResponse>('setup.status', {}).then(setup => {
        // A25：未配置时提示继续（与 startNewSession 同策略）
        if (setup?.provider_configured === false) {
          sys('未配置模型密钥——请用 /model set-key <密钥> 配置（配置类命令不受影响）')
        }
