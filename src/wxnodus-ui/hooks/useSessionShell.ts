...
import { createGatewayEventHandler } from '../bridge/eventAdapter.js'
import { createSlashHandler } from '../commands/slashHandler.js'
...
import { $uiState, getUiState, patchUiState } from '../runtime/viewStore.js'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(
    model ? composeTabTitle(marker, ui.sessionTitle, model, tabCwd ? shortCwd(tabCwd, 24) : '') : 'WxNodus'
  )
  ...
  const cwd = ui.info?.cwd || process.env.WXNODUS_CWD || process.cwd()
