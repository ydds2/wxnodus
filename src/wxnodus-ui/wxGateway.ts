...
import type { EventBus, WxEvent } from '../kernel/events.js'
import type { CommandBus, ExecResult } from '../app/CommandBus.js'
import type {
  RunInvocationHandle,
  RunInvocationPort,
} from '../application/runs/runInvocationPort.js'
import type { CoordinatedRunResult } from '../application/runs/sessionRunCoordinator.js'
...
export interface WxGatewayKernel {
  ...
  /** 顶层前台 Agent/Command Run 的唯一接纳端口。 */
  ...
  /** 现代 process-backed 子代理的权威活动表与定向终止端口。 */
  delegateManager?: import('../application/autonomy/delegateManager.js').DelegateManager
...
export class GatewayClient extends EventEmitter {
  ...
  private isForegroundEvent(event: WxEvent): boolean {
    return this.foregroundRun?.context.runId === event.runId
  }

  private reserveForeground(): number {
    if (this.running) {
      const error = new Error('session busy: waiting for active run') as Error & { code: number }
      error.code = 4009
      throw error
    }
    this.running = true
    return ++this.foregroundAdmission
  }

  private assertForegroundReservation(admission: number): void {
    if (!this.running || this.foregroundAdmission !== admission) {
      const error = new Error('foreground admission cancelled') as Error & { code: number }
      error.code = 4009
      throw error
    }
  }

  private releaseForegroundReservation(admission?: number): void {
    if (admission !== undefined && this.foregroundAdmission !== admission) return
    if (!this.foregroundRun) this.running = false
  }

  private cancelForeground(): boolean {
    if (!this.running) return false
    if (this.foregroundRun) {
      this.foregroundRun.cancel()
      return true
    }
  ...
  private bindForegroundRun<T>(handle: RunInvocationHandle<T>): Promise<CoordinatedRunResult<T>> {
    this.foregroundRun = handle as RunInvocationHandle<unknown>
    return handle.completion.then(result => {
      if (this.foregroundRun?.context.runId === handle.context.runId) {
        this.foregroundRun = null
        this.running = false
      }
      return result
    }, cause => {
      ...
      const message = String((cause as Error)?.message ?? cause).slice(0, 300)
      this.publish({ type: 'error', payload: { message } })
      throw cause
    })
  }
  ...
  private attachBus() {
    const foregroundEventTypes = new Set([
      'agent.start',
      'agent.token',
      'agent.message',
      'reasoning.delta',
    ...
    const map: Record<string, (p: any, e: WxEvent) => void> = {
      'agent.start': (p) => {
        this.running = true
      ...
      // A25：子代理思考分流（按活跃子代理稳定 ID 判定 → subagent.thinking 富事件——
      // 此前子代理推理混入主面板或直接丢失，agentsOverlay 思考区恒空）
      'reasoning.delta': (p) => {
        const text = String(p?.text ?? '')
        if (!text) return
        const sid = String(p?.session_id ?? '')
        if (this.activeSubagents.has(sid)) {
          this.publish({
      ...
      'agent.tool': (p) => {
        ...
        // A25：子代理工具分流（按活跃子代理稳定 ID 判定 → subagent.tool 富事件——
      ...
      'agent.end': () => {
        // agent.end 只表示一次 Agent 调用结束；权威 Run 终态由匹配的 run.final 清除 busy。
      ...
      'run.final': (p, e) => {
        if (!this.isForegroundEvent(e)) return
        this.foregroundRun = null
        this.running = false
        const status = String(p?.status ?? 'failed')
        this.publish({
          type: 'status.update',
          payload: { kind: 'done', text: status === 'succeeded' ? 'ready' : status },
        })
      },
      'system.notice': (p) => {
      ...
      'jobs.complete': (p, e) => {
        ...
        if (!this.backgroundTaskIds.delete(taskId)) return
    ...
    for (const [type, fn] of Object.entries(map)) {
      this.unsubscribe.push(this.kernel.bus.on(type, (e) => {
        if (foregroundEventTypes.has(type) && !this.isForegroundEvent(e)) return
        fn(e.payload, e)
      }))
    }
  }
  ...
  start() {
    this.attachBus()
    this.currentSessionId = this.kernel.adapter.agent.getSessionId()
  ...
  private cwdSet(params: Record<string, unknown>): unknown {
    ...
    try {
      ...
      return {
        ok: false,
        error: `运行中不能切换工作区：请执行 /workspace set ${p}，然后重启 WxNodus`,
  ...
  private delegationStatus(): unknown {
    const active = new Map<string, Record<string, unknown>>([...this.activeSubagents].map(id => {
      const detail = this.activeSubagentDetail.get(id)
      return [id, detail
        ? { subagent_id: id, goal: detail.goal, started_at: detail.started_at, status: detail.status }
        : { subagent_id: id }]
    }))
    for (const task of this.kernel.delegateManager?.listActive() ?? []) {
      active.set(task.taskId, {
    ...
    return {
      ...
      paused: this.kernel.delegateManager?.isPaused()
        ?? this.kernel.adapter.agent.getDelegationPaused?.()
        ?? false,
    }
  }
  ...
  private async runForegroundCommand(input: string, sessionId: string, admission: number): Promise<CoordinatedRunResult<ExecResult>> {
    try {
      this.assertForegroundReservation(admission)
      const handle = this.kernel.runInvocation.invoke({
        kind: 'command',
        command: input,
        sessionId,
        beforeRelease: run => {
          if (run.status === 'succeeded' && run.activeSessionId) {
            this.currentSessionId = run.activeSessionId
          }
        },
      })
      return await this.bindForegroundRun(handle)
    } catch (cause) {
      this.releaseForegroundReservation(admission)
      throw cause
    }
  }

  private async runForegroundSession<T>(request: {
    sessionId: string
    targetSessionId?: string
    operation(signal: AbortSignal): Promise<T> | T
    classify(value: T): RunFinalStatus
  }): Promise<CoordinatedRunResult<T>> {
    const admission = this.reserveForeground()
    try {
      this.assertForegroundReservation(admission)
      const handle = this.kernel.runInvocation.invoke({
        kind: 'session',
        ...
        operation: ({ signal }) => request.operation(signal),
  ...
  private async dispatchCommand(params: Record<string, unknown>): Promise<unknown> {
    ...
    const run = await this.runForegroundCommand(input, sid, admission)
    const r = run.value

    if (!r) return { type: 'exec', output: run.error ?? `命令以 ${run.status} 结束` }
  ...
  // P1c：plugins.manage RPC 使用组合根唯一插件快照；toggle 失败恢复原清单与旧 runtime。
  private async pluginsManage(params: Record<string, unknown>): Promise<unknown> {
    const { setPluginEnabled } = await import('../kernel/plugins.js')
    const action = String(params.action ?? 'list')
    const name = String(params.name ?? '')

    if (action === 'list') {
      const all = this.kernel.getPlugins?.() ?? []
      return {
        plugins: all.map(p => ({
    ...
    if (action === 'toggle') {
      ...
      if (!existsSync(manifestFile) || !this.kernel.reloadPlugins) return { plugin: null }
      const previous = readFileSync(manifestFile, 'utf8')
      if (!setPluginEnabled(dir, enable)) return { plugin: null }
      const reloaded = await this.kernel.reloadPlugins()
      if (!reloaded.ok) {
        writeFileSync(manifestFile, previous, 'utf8')
        await this.kernel.reloadPlugins()
        return { plugin: null, error: reloaded.message }
      }
  ...
  private async promptSubmit(params: Record<string, unknown>): Promise<unknown> {
    ...
    try {
      ...
      // 顶层前台 Agent Run 经统一接纳端口执行；RPC 保持非阻塞，结果由事件流投影。
      this.assertForegroundReservation(admission)
      const handle = this.kernel.runInvocation.invoke({
        kind: 'agent',
        prompt: finalText,
        sessionId: sid,
        options: images ? { images } : undefined,
      })
      void this.bindForegroundRun(handle).catch(() => { /* bindForegroundRun 已发布错误 */ })

      return { ok: true, run_id: handle.context.runId }
    } catch (cause) {
      this.releaseForegroundReservation(admission)
      throw cause
    }
  ...
  private async slashExec(params: Record<string, unknown>): Promise<unknown> {
    ...
    try {
      ...
      if (classifyCommand(full) === 'danger' && params.confirm !== true) {
        ...
        if (choice === 'deny') {
          this.releaseForegroundReservation(admission)
          return { output: '', warning: `危险命令已取消（未执行）：${command}` }
        }
      }
      const sid = String(params.session_id ?? this.currentSessionId)
      const run = await this.runForegroundCommand(full, sid, admission)
      const r = run.value
      if (!r) {
        return { output: '', warning: run.error ?? `命令以 ${run.status} 结束` }
      }
  ...
  private async sessionCreate(_params: Record<string, unknown>): Promise<unknown> {
    ...
    const run = await this.runForegroundSession({
      ...
      operation: async () => {
        ...
        if (!ensured.ok) return { ok: false as const, message: `会话工件生成失败：${ensured.code}` }
        try {
      ...
      classify: value => 'ok' in value && value.ok === false ? 'failed' : 'succeeded',
    })
    if (!run.value) return { ok: false, message: run.error ?? `会话创建以 ${run.status} 结束` }
    if ('ok' in run.value && run.value.ok === false) return run.value
    return { ...run.value, info: this.buildInfo() }
  }
  ...
  private async sessionActivate(params: Record<string, unknown>): Promise<unknown> {
    ...
    const run = await this.runForegroundSession({
      ...
      operation: async () => {
        ...
        const ensured = await this.kernel.adapter.data.sessions.ensure(id)
        if (!ensured.ok) return { ok: false as const, message: `会话工件校验失败：${ensured.code}` }
        return { session_id: id, messages: this.loadMessages(id), running: false, started_at: Date.now() / 1000 }
      },
      classify: value => 'ok' in value && value.ok === false ? 'failed' : 'succeeded',
    })
    if (!run.value) return { ok: false, message: run.error ?? `会话激活以 ${run.status} 结束` }
    if ('ok' in run.value && run.value.ok === false) return run.value
  ...
  private async sessionResume(params: Record<string, unknown>): Promise<unknown> {
    ...
    if (!run.value) return { ok: false, message: run.error ?? `会话恢复以 ${run.status} 结束` }
    if ('ok' in run.value && run.value.ok === false) return run.value
  ...
  private async sessionClose(params: Record<string, unknown>): Promise<unknown> {
    ...
    return run.value ?? { ok: false, message: run.error ?? `会话关闭以 ${run.status} 结束` }
  }
  ...
  private async sessionDelete(params: Record<string, unknown>): Promise<unknown> {
    ...
    const run = await this.runForegroundSession({
      sessionId: id,
      ...(deletingCurrent ? { targetSessionId: 'default' } : {}),
      operation: () => {
        if (!this.kernel.adapter.data.sessions.exists(id)) {
          return { ok: false as const, message: `会话不存在：${id}` }
        }
    ...
    if (!run.value) return { ok: false, message: run.error ?? `会话删除以 ${run.status} 结束` }
    if (run.value.ok) this.publish({ type: 'status.update', payload: { kind: 'done', text: 'ready' } })
    return run.value
  }
  ...
  private async subagentInterrupt(params: Record<string, unknown>): Promise<unknown> {
    const taskId = String(params.subagent_id ?? '')
    if (!this.kernel.delegateManager) {
      return { ok: false, interrupted: false, message: `定向终止端口不可用：${taskId}` }
    }
    const result = await this.kernel.delegateManager.stop(taskId)
    return {
      found: result.ok || result.error !== 'SUBAGENT_NOT_RUNNING',
      subagent_id: taskId,
      ok: result.ok,
  ...
  private async delegationPause(params: Record<string, unknown>): Promise<unknown> {
    const paused = params.paused === undefined ? params.pause !== false : params.paused !== false
    // 暂停只拒绝新的委派；不得取消无关前台 Run 或已在执行的子代理。
    this.kernel.delegateManager?.setPaused(paused)
  ...
  // P3 修复：响应契约对齐 UI 消费端（core.ts 读 r.removed>0）
  private async sessionUndo(params: Record<string, unknown>): Promise<unknown> {
    ...
    const run = await this.runForegroundSession({
      ...
      operation: () => {
        ...
        if (!nonSys.length) return { ok: false as const, removed: 0, message: '没有可撤销的消息' }
        const userIdx = nonSys.map((m, i) => (m.role === 'user' ? i : -1)).filter(i => i >= 0)
        if (!userIdx.length) return { ok: false as const, removed: 0, message: '没有可撤销的轮次' }
        ...
        return { ok: true as const, removed: dropIds.length, deleted_id: dropIds[dropIds.length - 1] }
      },
      classify: value => value.ok ? 'succeeded' : 'failed',
    })
    const value = run.value ?? { ok: false, removed: 0, message: run.error ?? `会话撤销以 ${run.status} 结束` }
    if (value.ok) this.publish({ type: 'status.update', payload: { kind: 'done', text: 'ready' } })
    return value
  }
  ...
  private async sessionInterrupt(): Promise<unknown> {
    const interrupted = this.cancelForeground()
  ...
  private async sessionCompress(params: Record<string, unknown>): Promise<unknown> {
    const sid = String(params.session_id ?? this.currentSessionId)
    const admission = this.reserveForeground()
    const run = await this.runForegroundCommand('/compact', sid, admission)
    const command = run.value
    if (!command || !command.ok) {
      return {
        ok: false,
        message: command?.error ?? command?.output ?? run.error ?? `压缩以 ${run.status} 结束`,
      }
    }
    const rows = this.loadMessages(sid)
    // A24 第三类修复：返回真实 usage（含压缩计数）
    const info = this.buildInfo()
    return { ok: true, messages: rows, info, usage: info.usage }
  }
  ...
  private async sessionBranch(params: Record<string, unknown>): Promise<unknown> {
    ...
    const title = name || `${src} branch`
    const run = await this.runForegroundSession({
      sessionId: src,
      targetSessionId: newId,
      operation: () => {
        try {
          if (!this.kernel.adapter.data.sessions.branch(src, newId, title)) {
            return { ok: false as const, error: '会话不存在' }
          }
          return { ok: true as const, session_id: newId, title, messages: this.loadMessages(newId) }
        } catch (e: any) {
          return { ok: false as const, error: String(e?.message ?? e).slice(0, 120) }
        }
      },
      classify: value => value.ok ? 'succeeded' : 'failed',
    })
    if (!run.value) return { ok: false, error: run.error ?? `会话分支以 ${run.status} 结束` }
    if (!run.value.ok) return run.value
    return { ...run.value, info: this.buildInfo() }
  }
  ...
  private async promptBackground(params: Record<string, unknown>): Promise<unknown> {
    ...
    const runner = this.kernel.taskRunner
    if (!runner) throw new Error('background task runner unavailable')
    const taskId = runner.run({
      goal: text,
      kind: 'agent',
      tags: [`tui:${this.currentSessionId}`],
    })
  ...
  private async processStop(): Promise<unknown> {
    ...
    for (const taskId of [...this.backgroundTaskIds]) {
      ...
      if (await runner.kill(taskId)) killed++
    }
    return { killed }
  }
  ...
  private async routeVoiceSecret(text: string): Promise<boolean> {
    ...
    const storeSecret = async (secret: string): Promise<void> => {
      const admission = this.reserveForeground()
      const run = await this.runForegroundCommand(`/model set-key ${secret}`, this.currentSessionId, admission)
      const command = run.value
      if (command?.ok && run.status === 'succeeded') {
      ...
      this.publish({
        ...
        payload: { text: `密钥录入失败：${command?.error ?? run.error ?? run.status}`, level: 'error' },
      })
    }
    ...
    if (this.voicePendingSecret) {
      ...
      await storeSecret(hit.secret || text)
      return true
    }

    if (hit.secret) {
      // 转写直接含密钥（"设置密钥 sk-xxx" / "sk-xxx"）→ 本地加密存储
      await storeSecret(hit.secret)
    } else {
