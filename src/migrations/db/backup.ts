...
export function restoreDbFromBackup(dbPath: string, backup: DbBackup): { ok: true } | { ok: false; code: string; message?: string } {
  const checksum = verifyDbBackup(backup);
  if (!checksum.ok) return checksum;
  ...
  // V4 P3-6（B-4）：WAL 卫生——替换主库前删 -wal/-shm 残留（SQLite howtocorrupt 明列的
  // 替换主库留旧 WAL 损坏路径：残留 WAL 下次打开按内部校验和重放旧页到新库）
  for (const suffix of ['-wal', '-shm']) {
    try { rmSync(dbPath + suffix, { force: true }); } catch { /* 清理失败继续 */ }
  }
  if (existsSync(dbPath)) rmSync(dbPath);
  try {
    renameSync(tmp, dbPath);
  } catch (cause) {
    return { ok: false, code: 'DB_RESTORE_RENAME_FAILED' as const, message: `恢复替换失败：${String((cause as Error)?.message ?? cause)}` };
  }
  return { ok: true };
}
