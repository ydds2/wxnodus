...
import type { SubagentDefinition, SubagentRunOptions } from '../kernel/subagentTypes.js';
...
export interface HandlerCtx {
  ...
  agent?: {
    ...
    setScriptRecorder?(fn: ((name: string, args: Record<string, any>) => void) | null): void;
    runScript?(steps: any[]): Promise<{ ok: boolean; log: Array<{ kind: string; step: number; text: string; name?: string }> }>;
  };
  getModel: () => string;
  getMode: () => string;
  setMode: (m: string) => void;
  ...
  /** 命令总线（非插件动态注册场景使用；插件注册由组合根 owner 持有） */
  commandBus?: CommandBus;
  /** 长驻协议后续请求的顶层 Run 接纳端口；已接纳命令不得同步等待其 completion。 */
  runInvocation?: import('../application/runs/runInvocationPort.js').RunInvocationPort;
  /** 命令启动的长驻资源注册到进程统一 shutdown；测试/嵌入环境可省略并显式 stop。 */
  registerDisposer?(id: string, dispose: () => Promise<void> | void): void;
  /** 组合根持有的现代子代理生命周期；命令、Gateway 与 shutdown 共享同一实例。 */
  delegateManager?: import('../application/autonomy/delegateManager.js').DelegateManager;
  /** 命令返回后仍持续运行并持有 delegateManager 的宿主（TUI/serve）；一次性 -p 必须为 false。 */
  liveDelegateHost?: boolean;
...
用户主题：${extras.join(' / ')}` : ''}`;
  ...
  // 当前版本只持久化并在下次启动原子生效，禁止同进程半热切换制造边界分裂。
  bus.register('/workspace', (args) => {
    ...
    if (sub === 'set') {
      ...
      const resolved = resolveWorkspaceRoot({ cli: dir, cwd: ctx.cwd });
      if (!resolved.ok) return `${resolved.error.code}：${String(resolved.error.details?.reason ?? resolved.error.message)}`;
      ctx.setWorkspace?.(resolved.value.value);
      return `主工作区已保存：${resolved.value.value}（重启 WxNodus 后原子生效；当前进程仍使用 ${ctx.workspaceRoot ?? ctx.cwd}）`;
    }
    if (sub === 'reset') {
      ctx.setWorkspace?.(null);
      return `主工作区重置已保存（重启 WxNodus 后使用启动目录；当前进程仍使用 ${ctx.workspaceRoot ?? ctx.cwd}）`;
    }
