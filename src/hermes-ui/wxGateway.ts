// src/hermes-ui/wxGateway.ts — Hermes UI ↔ wxnodus 内核桥接（进程内 GatewayClient）
// 设计：保持 Hermes GatewayClient 公开接口（start/request/drain/kill/getLogTail/on/off），
//       但不再 spawn python 子进程——直接路由到 wxnodus 内核（bus/db/config/mem/agent/commandBus）
//       wxnodus agent 事件 → Hermes GatewayEvent（message.delta/tool.*/status.update 等）
// 参考：Hermes gatewayClient.ts 接口契约 + wxnodus kernel/events 事件流
...
export class GatewayClient extends EventEmitter {
  ...
  // 事件翻译：wxnodus agent 事件 → Hermes GatewayEvent
  private attachBus() {
  ...
  private setupStatus(): unknown {
    ...
    // 始终放行，避免 Hermes 的 Setup Required 门禁阻塞本地可用的规则脑模式
