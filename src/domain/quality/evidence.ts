...
export interface EvidenceAttachmentRef { attachmentId: string; relativePath: string; path?: `attachments/${string}`; sha256: string; bytes: number }
...
export interface EvidenceRecord {
  ...
  /** W3-01：全量附件引用与闭合声明（W1-09 记录可不带——由 store 的 W1 布局隐含闭合） */
