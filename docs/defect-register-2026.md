...
| S-02 | 插件市场/远端技能安装 | codex/opencode 市场 | 公开托管已暂缓（用户决策） | ⑧ 6→7（+9） | ✅ **能力面闭环（消费侧）**：`/plugin install`（SSRF+校验+sha256+staging，10 单测）+ **`/market` 开放生态目录聚合**（1e9380d：npm registry + GitHub topic 双源搜索、mcp→.mcp.json / skill→data/skills 安装路由、域名白名单+注入式 fetch 可测，7 单测）——**不建自托管目录，兼容全网开放资源即市场**；⑧ 6→7 计分仍受 5→6（分发）前置约束 |
