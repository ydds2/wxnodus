...
### 13.90 /build 复杂需求构造能力升级（2026-08-19：Spec v2 需求分解 + 逐模块生成引擎）

用户指摘「复杂需求场景下构造能力太弱，无法支撑超大型项目构建」——根因确认：规则脑移除后计划构造退化为**固定单模块计划**，5 种固定模具使跨域/多子系统需求全部坍缩进单模板。

1. **Spec v2（需求分解）**：`Spec.modules?` 可选模块 DAG（`{name, deps, desc, files[{path,desc}]}`）——AI 规格化对复杂需求输出 1~8 模块分解（每模块 ≤12 文件），简单需求省略该字段走模具模板（向后兼容）；`validateSpec` 新增结构校验：模块名白名单、依赖必须指向分解内模块、禁自依赖、依赖环（topoSort 抛错即拒绝）、文件路径白名单（禁 `..`/绝对路径/大写）、数量上限。
2. **逐模块生成引擎（`src/build/generate.ts`）**：拓扑序每模块**一次 LLM 调用**生成该模块全部文件（有界多轮——单次输出上限不再约束项目总规模；每模块单文件 ≤64KB、模块 ≤192KB、≤12 文件）；三重校验（路径白名单/尺寸/重复）失败即诚实报错不落半成品；已生成模块文件路径作为后续模块上下文（相对 require 互连）；**入口契约**：缺 `server/index.js` 如实失败（verifyProject 启动→探活→重启→读回闭环不变）；项目脚手架（healthcheck/package.json/plan.json/README）与模板路径同形态。
3. **/build 接线**：计划 = AI 分解 DAG（`topoSort`）而非硬编码单模块；instantiate 按 `spec.modules` 分派生成/模板；**超时按模块规模伸缩**（300s + 每模块 120s，上限 15min）；进度流逐模块 `system.notice`；`--dry-run` 展示模块 DAG 与文件清单；完成输出列模块拓扑。
4. **测试（8 用例）**：三重校验纯函数（穿越/绝对/非法扩展/重复/超尺寸）；Spec v2 结构校验（自依赖/未知依赖/环/上限）；mock fetch 端到端逐模块生成（拓扑落盘 + plan.json + 进度序 + 模块互连引用）；入口缺失诚实失败；JSON 非法不落半成品。既有 build/llmSpec/spec/commands 30 用例回归全绿（无 modules 的 Spec 走模板路径——兼容不变）。
...
**尾注（2026-08-19）**：本地九步门禁全绿；远程 CI #32262466159 全绿（`fad18e4`）。

### 13.93 缺失与不真实部分收口轮（2026-08-19：/encrypt 真实加密 /render 真实渲染 /rollback 真 diff / 树面板形态）

按用户指摘「继续完善其他缺少、不真实部分」（公开决策外）逐项处置：

1. **/encrypt 不真实修**：此前仅凭证状态展示却挂「加密工具」描述——现真实文件加解密：`src/kernel/fileCrypto.ts`（scrypt 口令派生 + AES-256-GCM 认证加密，`WXENC1|salt|iv|tag|密文` 封装；口令 `--key` 参数或 `WXNODUS_ENC_KEY` 环境变量不落盘不回显）；`/encrypt file <路径> --key <口令>` → `<路径>.enc`、`/encrypt decrypt` 反向；错误口令/篡改 → GCM 认证诚实拒绝。5 用例（往返/错误口令/篡改/非格式/随机盐）。
2. **/render 不真实修**：此前行级前缀变换却挂「Markdown 排版预览」——现复用成熟解析器（micromark+GFM+math，与 TUI 渲染同源）：`src/wxnodus-ui/lib/markdown/renderText.ts`（标题/列表/代码块/表格列对齐/引用/公式/分隔线真实渲染）。顺带修复 `parseMd` 流式容错缺陷：围栏**奇数才补全**（此前偶数已闭合且结尾为围栏行也误补——完整输入产生多余空代码块，实测发现并修）。4 用例。
3. **rollbackDiff 不真实修**：此前快照/当前全文等值才判一致、否则两端整段对贴——现 `lineDiff` 真实 diff（+/- 行级呈现）。
...
### 13.94 UI 重设计规划（2026-08-19：四层布局 + 栈式浮层 + 键位注册表 + 工作台化路线）
...
**尾注（2026-08-20）**：两项各自全量单测绿 + tsc 干净后提交；本地九步门禁全绿。远程 CI #32280768977 首轮 shard1 红——`tests/kernel-agent.test.ts` 读工具缓存用例 60s 超时（分片总耗时 340s，负载抖动；本地同用例 65/65 全绿 24s，与本轮 UI 改动无关）；`gh run rerun --failed` 重跑后 **success**（37f1361 全树验证通过，含 P1-1~P1-4 全部代码——中间 780b2dd/6c972fe 两轮因连续推送被 concurrency 取消，属设计内）。如实入档不掩盖。

### 13.98 UI 重构 P2 执行实录（2026-08-20：面板右分栏 + 状态栏零记忆入口全覆盖）

用户「继续」——P2 批两项（`a26fac2` + `e74db38`）：

1. **面板右分栏**（`a26fac2`）——P1 遗留的最大布局变更落地：面板组（config/model/skills/plugins——互斥组保证至多 1 个）迁出浮层：宽窗（≥80 列）渲染为**右侧分栏**（宽度 `min(40, cols-50%)`，不遮转录流；主列/状态栏如实收窄——布局真重组而非遮罩）；小窗自动降级为转录流与输入区之间的全宽块（不再浮层遮蔽，诚实降级不假自适应）。`RightPanelPane` 单一渲染源（gw/sid/onSelect/onClose 与既有 FloatBox 版逐 prop 对齐），appOverlays 移除 4 个面板 FloatBox + onModelSelect 通路（AppOverlaysProps 同步收窄）；`findPanelKind` 纯函数 + 契约测试。全量 2894 零回归。
2. **状态栏零记忆入口全覆盖**（`e74db38`）：既有可点段（会话计数→sessions / 模型→modelPicker / 目录→dirPicker / 余额→刷新 / token 区间→轮换 / 语音→开关）之外新增 **⚙ 配置入口**（最低优先级尾段，窄屏最先让位）直达右分栏配置面板。
...
**远程 CI 环境故障实录（2026-08-20）**：复查提交 60b532a 的远程运行 #32290870489 首轮 failure——两 job（gate/vscode-ext）在 ~3s 内失败且 **steps 为空**（runner 从未注册步骤）、日志 blob 缺失；连重跑 4 次（attempt 2–4）与全新 workflow_dispatch #32291912771 均同秒失败。**决定性证据**：重跑 20 分钟前成功的 #32288629808（1b5e337，同一工作流同一树）同样秒失败——失败与代码/提交无关，系 GitHub hosted `windows-latest` runner 供给故障（repo Actions 权限 enabled、GitHub 官方状态页「All Systems Operational」、工作流文件自上次成功后未变）。本地九步门禁在同树全绿（与远程命令同源）。处置：如实登记，待 runner 供给恢复后重触发；**不假绿、不掩盖**。

**自托管 runner 撤销实录（2026-08-20，用户边界声明后即日执行）**：在 hosted 故障排查中，曾按用户选择部署本机 self-hosted runner（wxnodus-local-win）并切换 ci.yml runs-on——但用户随后明确边界：「只允许 wxnodus 接收（下载）、自己构建，不允许 GitHub 连接本机」。立即执行：runner 注销（remove-token + credentials/.runner 清理 + 进程终止 + 目录删除）、ci.yml 恢复 hosted windows-latest 原样（`68c930c`）。诚实结论：远程 CI 在 hosted 配额恢复前不可用——门禁以本地九步 `npm run ci` 为准（命令与远程同源，audit 尾注如实标注远程断档）。GitHub 连接仅剩「git push 代码备份」方向（用户长期默许；如需停止推送亦属用户决定，另行声明即执行）。
