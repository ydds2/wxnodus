...
| 3.2 | **Windows 双态沙盒**（提权受限令牌 + 标准用户 Low IL，探测双态如实报告） | S-07 | 管理员环境（非阻塞） | 标准用户分支实测；提权分支探测诚实 | ✅ 提权分支实现 + 双态探测（实测待管理员环境——如实标注） |
...
| 3.6 | 超越复评：11 维评分 ≥ 870 | — | 全部 | score 更新 | ◐ 复评 790（未达 870——剩余增量卡 git remote/管理员环境/④⑤ 残留，见 score §9.11） |
