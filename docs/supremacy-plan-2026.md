...
### 阶段 1「内核登顶」（0 外部依赖）→ 725 → ~790
| # | 任务 | 缺陷 ID | 对标 | 验证 |
|---|---|---|---|---|
| 1.1 | 分族提示词（providerPrompts.ts 新模块） | A-02 | gemini | 单测：各 provider 段注入正确 |
| 1.2 | 小模型任务档（标题/摘要路由） | A-03 | crush large/small | 路由纯函数单测 |
| 1.3 | 按模型工具裁剪 | A-04 | codex | 裁剪集单测 |
| 1.4 | 成本五维 + Decimal（usage_stats v10） | A-06 | opencode | kf-030 同步 + 成本测试 |
| 1.5 | LLM 辅助循环检测（开关默认关） | A-05 | gemini | agent 流测试 |
| 1.6 | 命令面瘦身 109→~45 | A-01 | gemini 47 | 命令契约定向全绿 |
| 1.7 | execpolicy 首词规则 + 审批持久化 | B-06 | codex | 规则匹配单测 |

### 阶段 2「生态上车」（需 git remote 与桌面端决策）→ ~790 → ~840
| # | 任务 | 缺陷 ID | 前置 | 验证 |
|---|---|---|---|---|
