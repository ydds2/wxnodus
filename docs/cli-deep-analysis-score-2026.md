...
| **加权总分（/1000）** | **825** | **869** | **812** | **841** | **709** | **693** | **573** |

**排名：codex 8.69 ＞ opencode 8.41 ＞ wxnodus 8.25 ＞ gemini 8.12 ＞ crush 7.09 ＞ kimi 6.93 ＞ aider 5.73。**（wxnodus 由 6.14→7.25→7.54→7.90→8.14→8.25：第 3/7 稳固；⑦ 满格后距 opencode 841 差 16、距 codex 869 差 44）

**≥3 维度第一已达成（2026-08-18 补轮）：① 渲染 10、④ Agent 10、⑪ 差异化 8——三项均为七家第一。**
...
- **剩余 56 分去向（全部外部前置）**：⑥ 9→10（+10，管理员环境实测提权分支——用户已确认有管理员终端，待跑探测）；⑦ 9→10（+11，marketplace 上架/完整 exec-server，依赖 git remote）；⑧ 5→9（+36，winget/scoop 发布/市场托管，依赖 git remote）。**远程类任务按用户决策「暂无 remote，跳过发布类」保持阻塞**——870 线在此前提下不可达，如实记录。
...
- **剩余（全部外部前置）**：⑥ +10（管理员实测——脚本已交付等待回报）；⑧ +36（git remote 发布通道——用户已决策跳过）。两前置完成后：825+10+36 = **871 超 codex**。
