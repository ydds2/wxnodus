...
| **加权总分（/1000）** | **931** | **869** | **812** | **841** | **709** | **693** | **573** |

**排名：wxnodus 9.31 ＞ codex 8.69 ＞ opencode 8.41 ＞ gemini 8.12 ＞ crush 7.09 ＞ kimi 6.93 ＞ aider 5.73。**（wxnodus 由 6.14→…→9.22→**9.31（七评，② 9→10）**：三波路线 + P3 评估轮全部落定，稳居第 1/7——不依赖公开决策）

**维度第一（2026-08-18 七评更新）：① 渲染 10、④ Agent 10、⑥ 安全 10、⑩ token 工程 10、⑪ 差异化 10 五项严格第一；② 输入 10（与 codex 并列）、⑤ 提示词 10、⑦ 场景 10（与 opencode 并列）三项并列第一。**
...
判词先行（七评更新）：**wxnodus 931 稳居第 1/7**——波 3（②vim 模态 / ③完整 diff 查看+per-hunk 回滚（六家皆无差异化）/ ⑪本地跨会话语义召回（六家独有）+ACP 接收入档）+ P3 评估轮（② 9→10：VISUAL 六家皆无 + 文本对象 codex 8/8 全覆盖（另独有 <>）+ / 搜索 + Ctrl-R redo）。五项严格第一（①④⑥⑩⑪）+ 三项并列第一（②⑤⑦）。剩余拉分项集中在「产品化完成度」：分发（⑧ 5，卡公开决策——转公开即 +36）。
...
### 9.20 七评（2026-08-18：② 9→10 = 931——codex 文本对象 8/8 全覆盖复核，冲 10 条件闭合）

用户再次要求全量评估——七评先解决 §9.19 留下的唯一悬念（codex 八种文本对象覆盖复核）：

1. **codex 八种取证**：`vim.rs:52-61` VimTextObject = Word / BigWord / Parentheses / Brackets / Braces / DoubleQuote / SingleQuote / **Backtick**——此前 wxnodus 覆盖 7/8（缺反引号）+ 独有 <> 对。
2. **反引号对象补全（文本对象扩展，1 用例）**：textObjectRange 引号分支纳入 `` ` ``——di\`/da\` 同行配对。至此 **codex 八种 8/8 全覆盖**，另有 codex 没有的 `<>` 对象 + VISUAL 模式（codex VimMode 仅 Normal/Insert）+ / 搜索 + Ctrl-R redo——② 的 codex 差距完全闭合。
3. **② 9→10 判分**：输入/编辑器维度不再有任何 codex 独有的能力缺口（八种文本对象全覆盖、Ctrl-R 历史搜索 wave 1 已有、键位层/@补全/外部编辑器/token 高亮齐备）；wxnodus 侧额外独有 VISUAL 与 / 搜索。**+9 → 931**。
4. **其余维度复评不变**：③ 8（opencode 三源已对标落 git/branch，per-hunk 回滚仍独有；快照源 turn 保留）、⑨ 9、⑧ 5 卡公开决策、⑥ 10（§9.14 判分在案）。⑤⑦⑩⑪⑪①④ 维持。
5. **复算**：922 → **931**（第 1/7 不变，与 codex 869 差距拉大到 62）；严格第一 ①④⑥⑩⑪ 五项；并列第一 ②⑤⑦ 三项（② 与 codex 并列）。
6. **诚实口径**：主题预设 10 套（非 opencode 33）；B-03 会话浏览器 UI 与 C-02 wxGateway 拆分如实留存；ACP cancel 诚实报错不假装；/diff revert 仅 turn 源。
