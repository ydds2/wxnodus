...
| 3 | 补全 RPC 往返窗口内击键被吞（快打丢字） | cmd-verify 陷阱（200ms/字符规避） | ✅ 已修（2026-08 复测 5ms/字符双管线零丢字——parse-keypress 批次读取修复根治；电池保留 200ms 作 CI 确定性余量） |
| 4 | Esc 关闭 overlay 后 1.5s 输入恢复窗口失效 | full-scene 陷阱 5 | ✅ 已修（TextInput 常驻挂载：阻断期 display:none + focus=false 注销监听，关闭即恢复——根除卸载重挂的监听注册窗口） |
| 5 | CJK 高速键入丢字竞态（pty 直输） | full-scene 陷阱 4 | ✅ 已修（2026-08 复测：CJK 与 ASCII 5ms/字符双管线零丢字；IME 真机通道同步验证无损） |
...
| 10 | Esc-Esc 语义与 Claude/Gemini 相反（中断确认 vs 回滚） | 竞品对照 | ✅ 已修（arm 提示明示「中断」+ 中断后即指路 /undo 回滚——Claude 回滚肌肉记忆的等价出口） |
| 11 | 无 /cost 会话成本累计 | 竞品对照 | ✅ 已修（/cost [session\|today\|7d\|30d] 会话级 $ 估算 + 状态栏 📊 token 区间 + 💰 余额监控 + 会话面板 $ 列；未收录定价诚实省略、端点未上报用量 ⚠N 标注——参考公开价目，非账单） |
...
- **UX 债清仓**：12 项缺陷清单全部关闭（#3/#5 复测零丢字口径修正；#4 TextInput 常驻挂载根治；#7 diff 高亮；#8 @提及；#9 键位；#10 Esc-Esc 回滚指路；#11 会话级 /cost 已补齐——会话/区间/状态栏/会话面板同源成本估算，未上报用量显式标注）。剩余已知边界：#12 首启语言选择（有 WXNODUS_LANG 逃生门，可接受）。
