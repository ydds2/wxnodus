...
- **UX 债清仓**：12 项缺陷清单全部关闭（#3/#5 复测零丢字口径修正；#4 TextInput 常驻挂载根治；#7 diff 高亮；#8 @提及；#9 键位；#10 Esc-Esc 回滚指路；#11 余额/token 部分覆盖、会话级 /cost 命令见下文）。剩余已知边界：#11 会话级 $ 成本命令待补、#12 首启语言选择（有 WXNODUS_LANG 逃生门，可接受）。
